export async function onRequestGet(context) {
  ...
  // Traer el texto completo de la entrada (no solo el resumen)
  let textoCompleto = item.resumen || "";
  try {
    const detalleRes = await env.ASSETS.fetch(
      new URL(`/data/mundogl/${item.id}.json`, request.url)
    );
    if (detalleRes.ok) {
  ...
  const titulo = `${item.tituloSeo || item.titulo} - Girls Love Play`;
  const descripcion = item.resumen || "Mundo GL en Girls Love Play";
  const imagen = item.img.startsWith("http")
    ? item.img
    : `https://girlsloveplay.com${item.img}`;
  const url = `https://girlsloveplay.com/mundo-gl/${params.slug}`;
  ...
  // 👇 mete el texto real, visible, en el HTML
  class InfoContenido {
    element(el) {
